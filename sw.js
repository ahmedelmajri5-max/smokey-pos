const CACHE_NAME = "smokey-pos-v65-ready-status-hotfix";
...
const FIREBASE_BRIDGE_SCRIPTS = `
  ...
  <script src="./firebase-ready-status-hotfix.js?v=1"></script>
  ${ENHANCEMENT_SCRIPTS}
`;
const FILES_TO_CACHE = [
  ...
  "./firebase-ready-status-hotfix.js?v=1",
...
async function injectFirebaseBridge(response) {
  ...
  if (html.includes("firebase-ready-status-hotfix.js?v=1")) {
    return new Response(html, { status: response.status, statusText: response.statusText, headers: response.headers });
  }
  const scripts = html.includes("firebase-order-reset.js?v=3") ? ENHANCEMENT_SCRIPTS + `\n<script src="./firebase-ready-status-hotfix.js?v=1"></script>` : FIREBASE_BRIDGE_SCRIPTS;
